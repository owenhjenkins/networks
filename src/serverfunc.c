	...
	while(connected){	
		...
		while(!http_read){
			...
			if(*buffer == '\n' && check_newline){		
				...
				// make the buffer bigger and add a null terminator
//				char *new_line = malloc(sizeof(char) * (backlog_size + 1));
//				if(!new_line){
//					error_thread("ERROR on malloc new_line");
//					connected = false;
//					break;				
//				}
//				bzero(new_line, (backlog_size + 1));
//				
//				if(backlog)strcpy(new_line, backlog);	
//		
//				if(backlog)free(backlog);			
//	
//				new_line[backlog_size-1] = '\0';
